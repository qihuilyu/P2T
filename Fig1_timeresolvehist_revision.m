...
for doserate = [0.1/60 1/60 10/60]
    for detectorefficiency = [1]
        ...
        det1 = find(detectorIds==200);
        
        det1_time = CorrectedTime(det1);
        det1_energy = energy(det1);
        
        
        [sorteddet1_time, sortInd] = sort(det1_time);
        sorteddet1_energy = det1_energy(sortInd);
        det1_timediff = diff(sorteddet1_time);
        sel_energy = sorteddet1_energy(2:end);
        
        figure;hist(det1_timediff(det1_timediff<1000000)/1000,30)
        
        xlabel('time since last detection (ms)')
        ylabel('count')
        
        thresh = 50000;
        ind_50000 = det1_timediff<thresh;        
        Nbins = 30;
        
          [N,Xedges,Yedges] = histcounts2(det1_timediff(ind_50000),sel_energy(ind_50000),Nbins);
           figure;heatmap(N)
 ax = gca;
ax.XData = thresh/Nbins/2:thresh/Nbins:thresh;
    ax.YData = 1/Nbins/2:1/Nbins:1;
    
        [N,Xedges,Yedges] = histcounts2(det1_timediff(ind_50000),sel_energy(ind_50000),[0:50000/Nbins:50000 0:1/Nbins:1]);
        figure;heatmap(N)
          
          
        Nbins = 30;
        X = [det1_timediff,sel_energy];
        figure;[N,c] = hist3(X,'Ctrs',{0:1/Nbins:5000 0:1/Nbins:1});
        figure;heatmap(c{2},c{1},N)
        
        
        figure;hist2d(det1_timediff,det1_energy)
         
        figure;hist2d(det1_time,det1_energy)
