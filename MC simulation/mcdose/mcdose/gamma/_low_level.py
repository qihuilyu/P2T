...
def _calculation_loop(**kwargs):
    ...
    if kwargs.get('output_components', False):
        output_components = True
        running_dd = np.inf * np.ones_like(kwargs['dose_evaluation'])
        running_dta = np.inf * np.ones_like(kwargs['dose_evaluation'])
    else:
    ...
    while True:
        ...
        dd_term = min_dose_difference / kwargs['dose_threshold']
        dta_term = distance / kwargs['distance_threshold']
        gamma_at_distance = np.sqrt( dd_term**2 + dta_term**2)

        update_mask = (gamma_at_distance < running_gamma[to_be_checked])
        running_gamma[to_be_checked] = np.where(update_mask, gamma_at_distance, running_gamma[to_be_checked])
        #  running_gamma[to_be_checked] = np.min(
        #      np.vstack((
        #          gamma_at_distance, running_gamma[to_be_checked]
        #      )), axis=0)
        if output_components:
            running_dd[to_be_checked] = np.where(update_mask, dd_term, running_dd[to_be_checked])
            running_dta[to_be_checked] = np.where(update_mask, dta_term, running_dta[to_be_checked])
    ...
    if output_components:
        return (running_gamma, running_dd, running_dta)
    else:
...
def _new_thread(kwargs, output, thread_index, gamma_store):
    if kwargs.get('output_components', False):
        dd_store = np.nan*np.ones_like(gamma_store)
        dta_store = np.nan*np.ones_like(gamma_store)
        gamma_store[thread_index], dd_store[thread_index], dta_store[thread_index] = _calculation_loop(**kwargs)
        if output is not None:
            output.put((gamma_store, dd_store, dta_store))
        else:
            return (gamma_store, dd_store, dta_store)
    else:
...
def calc_gamma(coords_reference, dose_reference,
               ...
               num_threads=1, output_components=False):
    """Compare two dose grids with the gamma index.
    Args:
    ...
    """
    #  num_threads = 1 # force single-threading to avoid memory leaks
    ...
    if output_components:
        dd_flat = np.nan * np.ones_like(dose_evaluation_flat)
        dta_flat = np.nan * np.ones_like(dose_evaluation_flat)
    ...
    if num_threads > 1:
        ...
        for thread_index in thread_indicies:
            ...
            p = Process(
                ...
                args=(
                    ...
                    np.nan * np.ones_like(dose_evaluation_flat)))
            p.start()
            processes.append(p)

        for i in range(num_threads):
            if output_components:
                res_gamma, res_dd, res_dta = output.get()
                thread_reference = np.invert(np.isnan(res_gamma))
                gamma_flat[thread_reference] = res_gamma[thread_reference]
                dd_flat[thread_reference] = res_dd[thread_reference]
                dta_flat[thread_reference] = res_dta[thread_reference]
            else:
                result = output.get()
        ...
        kwargs['dose_evaluation'] = dose_evaluation_flat[evaluation_index]
        kwargs['mesh_coords_evaluation'] = [coords[evaluation_index] for coords in coords_evaluation_flat]
        result = _new_thread(kwargs, None, evaluation_index, np.nan*np.ones_like(dose_evaluation_flat))
        if output_components:
            res_gamma, res_dd, res_dta = result
            reference = np.invert(np.isnan(res_gamma))
            gamma_flat[reference] = res_gamma[reference]
            dd_flat[reference] = res_dd[reference]
            dta_flat[reference] = res_dta[reference]
        else:
            reference = np.invert(np.isnan(result))
            gamma_flat[reference] = result[reference]
    ...
    if output_components:
        dd = np.reshape(dd_flat, np.shape(dose_evaluation))
        dta = np.reshape(dta_flat, np.shape(dose_evaluation))
        return (gamma, dd, dta)
    else:
        return gamma
