...
void NestedParam::ComputeTransformation(const G4int copyNo, G4VPhysicalVolume* physVol) const {
    // compute transformation of this voxel's center relative to the mother volumes center
    // x-position is aligned along repetition axis (can always set y and z to 0)

    // G4ThreeVector position((copyNo+1)*dx - dx*nx, 0.,0.);
    G4ThreeVector position((copyNo)*dx*2 - (nx-1)*dx , 0., 0.);
