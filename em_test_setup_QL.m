...
if ~isvar('xtrue'), printm 'xtrue'
	...
	im(4, mumap, 'attenuation map'), cbar
    set(gca, 'FontSize',FontSize)
...
if ~isvar('yi'), printm 'data yi'
	...
	im(2, proj, 'projections without attenuation'), cbar
    set(gca, 'FontSize',FontSize)
	im(5, ytrue, 'projections with attenuation'), cbar
    set(gca, 'FontSize',FontSize)
	clear ytrue proj
end
...
if ~isvar('xfbp'), printm 'fbp'
	...
	xfbp = max(xfbp, 0);
	im(6, xfbp, 'FBP with attenuation correction'), cbar
    set(gca, 'FontSize',FontSize)
    
    xfbp2 = em_fbp(sg, ig, yi);
	xfbp2 = max(xfbp2, 0);
	im(3, xfbp2, 'FBP without attenuation correction'), cbar
    set(gca, 'FontSize',FontSize)
end
